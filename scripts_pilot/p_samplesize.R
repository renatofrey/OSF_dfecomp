...
load("../objects/pilot/model_samplesize.Rdata")

pdf(width=10, height=4, file="../output/pilot/samplesize.pdf")
...
text(15, y.max/2, pos=3, "Sample size", cex=1.8, srt=90, xpd=T)
...
for (i in 1:4) {
  ...
  round_dec <- 0
  if (plot_aggr == T) {
    p.bars(round(subset(stat_perpart_long, mode == "solo" & cond == sel_cond)$samples, round_dec), side="l", panel="t")
    p.bars(round(subset(stat_perpart_long, mode == "comp" & cond == sel_cond)$samples, round_dec), side="r", panel="t")
  } else {
  ...
  axis(2, at=seq(0, y.max, length.out=6), las=1, mgp=c(1,0.6,0), cex.axis=1.2)
