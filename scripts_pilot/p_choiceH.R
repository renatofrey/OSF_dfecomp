...
load("../objects/pilot/model_H.Rdata")

pdf(width=10, height=4, file="../output/pilot/choiceH.pdf")
...
text(15, y.max/2, pos=3, "Proportion of H-choices", cex=1.8, srt=90, xpd=T)
...
for (i in 1:4) {
  ...
  round_dec <- 2
  if (plot_aggr == T) {
    p.bars(round(subset(stat_perpart_long, mode == "solo" & cond == sel_cond)$H, round_dec), side="l", panel="t")
    p.bars(round(subset(stat_perpart_long, mode == "comp" & cond == sel_cond)$H, round_dec), side="r", panel="t")
  } else {
