...
pdf(file="../output/pilot/environments.pdf", width=9, height=9)
par(mfcol=c(8,4), mar=c(2,3,2,1), mgp=c(0.5,.75,0))

for (i in 1:4) {
  if (i == 1) file_in <- "../data/pilot/dps_kind.csv"
  if (i == 2) file_in <- "../data/pilot/dps_w20.csv"
  if (i == 3) file_in <- "../data/pilot/dps_w15.csv"
  if (i == 4) file_in <- "../data/pilot/dps_w10.csv"
  ...
  for (dp in 1:8) {
    ...
    plot(H_hist, type="h", col="cyan", ylim=c(0,60), ylab=p_ylab, las=1, lwd=p_lwd, lend=1, xaxt="n", yaxt="n")
    points(L_hist, type="h", col="orange", lwd=p_lwd, lend=1)
    ...
    points(mean(H), ymax, pch=18, col="cyan", cex=1.5, xpd=T)
    points(mean(L), ymax, pch=18, col="orange", cex=1.5, xpd=T)
