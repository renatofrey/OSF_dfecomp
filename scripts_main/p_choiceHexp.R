...
pdf(width=10, height=4, file="../output/main/choiceHexp.pdf")
...
text(30, y.max/2, pos=3, "Choice sensitivity:\nProportion of Hexp-choices", cex=1.8, srt=90, xpd=T)
...
for (i in 1:3) {
  ...
  round_dec <- 2
  if (plot_aggr == T) {
    p.bars(round(subset(stat_perpart, mode == "solo" & cond == sel_cond)$Hexp, round_dec), side="l", panel="t")
    p.bars(round(subset(stat_perpart, mode == "comp" & cond == sel_cond)$Hexp, round_dec), side="r", panel="t")
  } else {
  ...
  for (sel_mode in c("solo", "comp")) {
    ...
    sel_estimates <- subset(est$Hexp, cond == sel_cond & mode == sel_mode)
