### This script summarizes the results of the prospective design analysis
### (c) Renato Frey

library(BEST)

ROPE_width <- 1
sel_effects <- c("condwick0.15", "condwick0.10", "modecomp", "condwick0.15:modecomp", "condwick0.10:modecomp")
...
files <- list.files("../objects/scicore/powersim_search/", full.names=T)
...
for (effect in sel_effects) {
  ...
  print(effect)
  pdf(file=paste("../output/main/powersim_search_", gsub("\\.", "_", effect), ".pdf", sep=""), height=4, width=9)
  ...
  for (file in files) {
    ...
    if (is.element(file, files[1:prod(par()$mfcol)])) {
      ...
      h <- hist(posterior, breaks=seq(-15, 15, by=.2), xlim=c(-10, 10 ), ylim=c(0, y.max), col=mycol, border=0, las=1, xlab="Sample size", ylab="Posterior samples (freq.)", main=paste("Simulation:", job))
...
pdf("../output/main/powersim_search.pdf", width=8)
...
title("Dependent variable: Sample size", adj=0.5, line=1.5, xpd=T, cex.main=1.5)
...
lab <- gsub("condwick0.15:modecomp", "Environment: Moderately wicked x Search mode: Competitive", lab)
lab <- gsub("condwick0.10:modecomp", "Environment: Extremely wicked x Search mode: Competitive", lab)
lab <- gsub("condwick0.15", "Environment: Moderately wicked", lab)
lab <- gsub("condwick0.10", "Environment: Extremely wicked", lab)
...
if (file.exists("../output/main/powersim_choice.pdf")) {
