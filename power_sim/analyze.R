...
ROPE_width <- 1
sel_effects <- c("condwick0.20", "condwick0.15", "condwick0.10", "modecomp", "gamble_ind")
...
files <- list.files("output/powersim/")
...
for (effect in sel_effects) {
  ...
  print(effect)
  pdf(file=paste("../output/main/powersim_", gsub("\\.", "_", effect), ".pdf", sep=""), height=4, width=9)
  par(mfrow=c(2,3), mar=c(5,5,2,2), mgp=c(2.7,.8,0))
  results_tmp <- NULL
  for (file in files) {
    
    load(paste("output/powersim/", file, sep=""))
...
pdf("../output/main/power.pdf", width=10)
...
text(.5, -3, "Proportion of simulation outcomes (power)", xpd=T)
text(-.075, mean(b), "Predictors / Sample size", srt=90, xpd=T)

legend(0, 22, xpd=T, pch=15, pt.cex=2, col=as.character(unlist(cols_cred)), c("Conclusive evidence (credible effect)", "Conclusive evidence (absence of effect)", "Inconclusive evidence"), box.lty=0)

#text(x=rep(0, length(b)/3), y=b[seq(length(b)-1, 1, by=-3)], xpd=T, sel_effects, pos=4, col="gray95")

text(x=rep(0, length(b)/3), y=b[seq(2, length(b), by=3)], xpd=T, c("Environment: Mildly wicked", "Environment: Moderately wicked", "Environment: Extremely wicked", "Search mode: Competitive", "Gamble index")[5:1], pos=4, col="gray95")
